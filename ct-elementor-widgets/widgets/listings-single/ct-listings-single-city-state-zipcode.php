 ...
 * @since 1.0.0
 */
class CT_Listings_Single_City_State_Zipcode extends Widget_Base {
	...
	public function get_name() {
		return 'ct-listings-single-city-state-zipcode';
	}
	...
	public function get_title() {
		return __( 'CT City, State & Zipcode', 'contempo' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'show_zipcode',
				[
					'label' => __( 'Show Zipcode', 'contempo' ),
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'label_on' => __( 'Show', 'contempo' ),
					'label_off' => __( 'Hide', 'contempo' ),
					'return_value' => 'yes',
			...
			$this->add_control(
				'show_country',
				[
					'label' => __( 'Show Country', 'contempo' ),
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'label_on' => __( 'Show', 'contempo' ),
					'label_off' => __( 'Hide', 'contempo' ),
					'return_value' => 'yes',
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .ct-elementor-listings-single-city-state-zipcode' => 'text-align: {{VALUE}}',
					],
				]
			);
		...
		$this->start_controls_section(
			...
			$this->add_group_control(
				...
				[
					...
					'selector' => '{{WRAPPER}} .ct-elementor-listings-single-city-state-zipcode',
					'fields_options' => [
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .ct-elementor-listings-single-city-state-zipcode' => 'color: {{VALUE}}',
					],
					'default' => '#121212',
				]
			);
	...
	protected function render() {
        ...
        $ct_country = strip_tags( get_the_term_list( ct_return_listing_id_elementor($attributes), 'country', '', ', ', '' ) );

		echo '<div class="ct-elementor-listings-single">';
			echo '<' . $settings['html_tag'] . ' class="ct-elementor-listings-single-city-state-zipcode">';
			if ( 'yes' === $settings['show_city'] ) {
				echo esc_html($ct_city);
	            echo '<span class="ct-elementor-listings-single-city-seperator">, </span>';
	        }
	        if ( 'yes' === $settings['show_state'] ) {
	            echo esc_html($ct_state);
	            echo '<span class="ct-elementor-listings-single-state-seperator"> </span>';
	        }
	        if ( 'yes' === $settings['show_zipcode'] ) {
	            echo esc_html($ct_zipcode);
	            echo '<span class="ct-elementor-listings-single--zipcode-seperator"> </span>';
	        }
	        if ( 'yes' === $settings['show_country'] ) {
	            echo esc_html($ct_country);
	        }
